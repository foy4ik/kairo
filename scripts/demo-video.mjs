...
await pp.addInitScript(() => localStorage.setItem('kairo-mock-db-v1', JSON.stringify({ settings: { language: 'ru', theme: 'dark', work_min: 25, short_break_min: 5, long_break_min: 15, long_break_every: 4, notifications: true, sound: false, auto_update: false, page_transitions: true, onboarded: true } })))
...
await page.addInitScript(() => {
  window.__KAIRO_FAKE_UPDATE__ = { version: '0.4.0', notes: '- Новые возможности и исправления\n- Улучшена производительность' }
  const add = () => {
...
await pause(500)
await type('Подготовить релиз 0.4 #docs !high')
await pause(400)
...
const card = page.getByTestId('task-card').filter({ hasText: 'Подготовить релиз 0.4' }).first()
...
// 4. Task panel: checklist, deadline, tag suggestions scoped to the project
await caption('Детали задачи без потери контекста: чек-лист, теги')
await click(page.getByTestId('task-card').filter({ hasText: 'Подготовить релиз 0.4' }).first())
...
await pause(700)
await caption('Подсказки тегов — только из этого проекта')
await click(panel.getByRole('combobox', { name: 'Добавить тег' }))
await type('f', 200)
await pause(1500)
await click(panel.getByRole('listbox').getByRole('option').first())
await pause(1300)
await page.keyboard.press('Escape')
await pause(700)

// 5. Smart sort of the whole board, with the explanation dialog
await caption('Сортировка по срокам и приоритету — с подтверждением')
await click(page.getByRole('button', { name: 'Сортировать всё' }))
const sortDlg = page.getByRole('dialog', { name: 'Сортировать всё' })
await sortDlg.waitFor()
await pause(2600)
await click(sortDlg.getByRole('button', { name: 'Сортировать' }))
await pause(2000)
await caption('Фильтр по статусу (колонке), тегам, сроку и приоритету')
await page.getByTestId('filter-status').selectOption({ label: 'В работе' })
await pause(1800)
await click(page.getByRole('button', { name: 'Сбросить' }))
await pause(900)

// 6. Notes: folders, linked to a task
await caption('Markdown-заметки: папки, теги, связь с задачами')
await click(page.getByRole('tab', { name: 'Заметки' }))
await pause(700)
await click(page.getByTestId('note-list').getByRole('button', { name: /Архитектура Kairo/ }))
await pause(1200)
...
await pause(900)
await glide(page.getByTestId('note-folder'))
await pause(1200)

// 7. Focus: own duration for this run only
await caption('Фокус: свои параметры на один запуск, настройки не меняются')
...
await pause(500)
await click(page.getByTestId('focus-params-toggle'))
await pause(600)
await click(page.getByTestId('focus-duration-input'), { dy: 0 })
...
// 10. Settings: smooth page transitions can be switched off; updates
await caption('Плавные переходы между вкладками — можно отключить в настройках')
