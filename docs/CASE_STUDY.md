...
Every version is a Git tag: `npm run release -- X.Y.Z` triggers a GitHub Actions workflow that builds installers for Windows, macOS and Linux, signs the update packages with a password-protected key and publishes a release plus the update feed (`latest.json`). Installed apps check the feed, ask the user, then download and verify the package. The whole flow was verified end to end: a v0.1.0 build found v0.1.1 on the live feed and offered it.

A 70-second demo ([kairo-demo.mp4](demo/kairo-demo.mp4)) is generated by a script from the real UI, so it can be regenerated after any change.
...
| Installer (NSIS, per-user, with the updater) | 3.2 MB |
| Application binary | 7.4 MB |
...
FTS5 for very large note libraries, a global hotkey for the timer, password-protected exports as a separate explicit feature, and code-signed installers (needs paid Microsoft / Apple certificates; auto-update itself is already in place).
